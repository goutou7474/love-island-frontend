...
前端设置页的“本机通知”会先读取 VAPID 公钥。服务器未配置时只展示提示，不会打断应用使用；配置完成后，浏览器订阅会保存到当前用户和当前小岛名下，并可直接从设置页发送测试提醒。后续生日、纪念日、悄悄话提醒复用这批订阅，不需要前端再改一套权限流程。
...
6. [x] 接真实统计聚合。
7. [x] 接年度报告生成。
8. [~] 接真实提醒：设备订阅、设置入口、测试发送和纪念日提醒脚本已完成，悄悄话/每日情话调度待接。

当前 MVP 已经覆盖 1-7。第 8 项已经完成订阅、测试发送和纪念日提醒基础设施，剩余提醒类型拆为 post-MVP 增强，不阻塞两个人正式使用。
